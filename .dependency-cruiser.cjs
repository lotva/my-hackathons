...
module.exports = {
	forbidden: [
...

		{
			...
			from: {
				path: String.raw`^src/\(modules\)/([^/]+)`,
			},
			name: 'enforce-public-api',
			severity: 'error',
			to: {
				path: String.raw`^src/\(modules\)/([^/]+)/(?!(?:index\.ts)$).+`,
				pathNot: String.raw`^src/\(modules\)/$1/`,
			},
		},
		{
			comment:
				'Core layer must not import from modules layer. Core should remain completely isolated and reusable.',
			from: {
				path: String.raw`^src/\(core\)`,
			},
...
// generated: dependency-cruiser@16.10.2 on 2025-06-06T19:57:16.165Z
